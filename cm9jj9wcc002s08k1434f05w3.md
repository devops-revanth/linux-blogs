...
```bash
bashCopyEditsudo chage -l viratkohli
```

**Output:**

```bash
pgsqlCopyEditLast password change                                    : Apr 16, 2025
...
```bash
bashCopyEditsudo chage [options] username
```
...
```bash
bashCopyEditsudo chage -M 45 viratkohli
```

#### 🔹 Set Minimum 7 Days Between Password Changes:

```bash
bashCopyEditsudo chage -m 7 viratkohli
```

#### 🔹 Warn User 5 Days Before Password Expiry:

```bash
bashCopyEditsudo chage -W 5 viratkohli
```

#### 🔹 Expire Password Immediately (Force Change on Next Login):

```bash
bashCopyEditsudo chage -d 0 viratkohli
```
...
```bash
bashCopyEditsudo useradd devplayer
```
...
```bash
bashCopyEditsu         # Switch to root user (requires root password)
...
```bash
bashCopyEditsudo command            # Run one command with root privileges
...
```bash
bashCopyEditcat /var/log/auth.log | grep sudo
```

or on some systems:

```bash
bashCopyEditjournalctl | grep sudo
```
...
```bash
bashCopyEditrishabpant ALL=(ALL) NOPASSWD: /bin/systemctl restart apache2
```
...
```bash
bashCopyEdit/var/log/auth.log     # Ubuntu/Debian
...
```bash
bashCopyEditsudo grep 'sudo' /var/log/auth.log
```
...
```bash
bashCopyEditsudo usermod -L username
```
...
```bash
bashCopyEditsudo grep john /etc/shadow
```

Look for a `!` like:

```bash
perlCopyEditjohn:!$6$XYZ...:...
```
...
```bash
bashCopyEditsudo usermod -U username
```
...
```bash
bashCopyEditsudo chage -E 2025-05-31 john
```
...
```bash
yamlCopyEditAccount expires              : May 31, 2025
Password inactive           : never
...
```bash
bashCopyEditsudo usermod --expiredate 1 john
```
...
```bash
php-templateCopyEdit<type>  <control>  <module_path>  <arguments>
```
...
```bash
bashCopyEditauth    required    pam_env.so
